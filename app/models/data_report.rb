  ...
  class TlTradeStatis
    def process(start_time)
      ...
      datas.each do |t|
        ...
        file.write("#{t.trade_time.strftime("%Y-%m-%d %H:%M:%S")},#{customer.try(:user).try(:phone)},#{customer.try(:customer_reg_info).try(:name)},#{customer.try(:customer_reg_info).try(:id_card)},#{t.jajin_log.amount},#{t.price}\r\n")
      end
      file.close
    end
  end

...
class MerchantLog < ActiveRecord::Base
  ...
  def process
    ...
    merchants.each do |item|
      ...
      merchant_log.name = item.sys_reg_info.sys_name
    ...
    agents = Agent.all
    agents.each do |item|
      merchant_log = MerchantLog.find_or_create_by(datetime: (now_day.to_date - 1.day) + 8.hours, data_type: "代理商")
      merchant_log.name = item.name

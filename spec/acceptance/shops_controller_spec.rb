...
resource "获取门店信息" do
  ...
  post "merchants/:merchant_id/shops"  do
    before(:each) do
      @agent = create(:agent)
      @agent.merchants << create(:merchant)
    end
    ...
    let(:merchant_id) { Agent.last.merchants.last.id }
  ...
  put "merchants/:merchant_id/shops/:shop_id"  do
    before(:each) do
      @agent = create(:agent)
    ...
    let(:merchant_id) { Agent.last.merchants.last.id }
    let(:shop_id) { Agent.last.merchants.last.shops.last.id }
